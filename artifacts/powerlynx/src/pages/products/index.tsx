  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-24">
      ...
      <div className="container mx-auto px-4 mt-8 flex flex-col md:flex-row gap-8 items-start">
        {/* SIDEBAR FILTERS */}
        <aside className="w-full md:w-64 shrink-0 space-y-8 sticky top-28">
          <div>
        ...
        <main className="flex-1 w-full">
          ...
          {filteredProducts.length === 0 ? (
            <div className="py-24 text-center bg-card border border-border border-dashed">
              <p className="text-lg font-bold text-muted-foreground uppercase tracking-widest mb-2">No Products Found</p>
              <p className="text-sm text-muted-foreground/70">Adjust your filters to see more results.</p>
            </div>
          ) : (
                ...
                <Link key={product.slug} href={`/products/${product.slug}`} data-testid={`link-product-${product.slug}`}>
                  <div className="group flex flex-col h-full bg-card border border-border hover:border-primary transition-all duration-300 hover:shadow-lg">
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      <div className="text-xs font-bold text-primary mb-2 uppercase tracking-wider line-clamp-1">{product.category}</div>
